...
export default function WinnersHistory({ elections, canDelete }: { elections: ClosedElection[]; canDelete: boolean }) {
  ...
  return <>
    <ul className="grid gap-4 sm:grid-cols-1 lg:grid-cols-1">
      {elections.map((election) => <li key={election.day}><button type="button" onClick={() => void open(election)} className="card h-full w-full text-left transition-transform hover:-translate-y-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-orange-600">
        <span className="status-badge status-won">Vote terminé</span>
