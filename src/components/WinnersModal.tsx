...
export default function WinnersModal({ election, result, loading, onClose, onRetry }: {
  ...
  return <Modal open onClose={onClose} title={`Résultats du ${formatElectionDate(election.voting_end)}`}>
    ...
    {loading ? <p role="status" className="rounded-2xl bg-white p-8 text-center">Chargement des gagnants…</p>
      ...
      : !result?.winners.length ? <div className="rounded-2xl border border-dashed border-stone-300 bg-white p-8 text-center"><h3 className="font-bold">Aucune attribution enregistrée</h3><p className="mt-2 text-sm text-stone-600">Le vote est terminé, mais aucun micro-ondes n’a encore été attribué pour cette journée.</p></div>
      : <ul className="space-y-4">{result.winners.map((winner) => <li key={winner.reservation_id} className="flex flex-col gap-5 rounded-2xl border border-stone-200 bg-white p-5 sm:flex-row">
        ...
        <div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-widest text-orange-700">Gagnant</p><h3 className="mt-1 break-words text-xl font-bold">{winner.pseudo}</h3>{winner.legende && <p className="mt-2 break-words text-sm text-stone-500">{winner.legende}</p>}<p className="mt-3 rounded-xl bg-orange-50 px-4 py-3 text-sm text-orange-950">Micro-ondes attribué : <strong>{winner.microwave_name}</strong></p></div>
      </li>)}</ul>}
  </Modal>;
}
